...
`useDark.ts` 用 **View Transition API** 做方向化圆扩散切换(`toggleDark` → `document.startViewTransition`)。`ensureViewTransitionStyles(toDark, x, y, r)` 每次 toggle 重新生成动态样式:① 点击坐标 `(x,y)` 与半径 `r` **直接焊进 `@keyframes` 字面量**(不走 `--var`)—— VT 伪元素在独立 box 树里,靠 `:root` 的 `--var` 继承脆弱,一旦断裂 clip-path 回退 `50%` → 圆从屏幕中心扩散(任何缩放都像"偏移");字面量坐标保证圆心=点击点。② **方向化**:Light→Dark 时旧(亮)快照向点击点**收缩**露出底层新(暗);Dark→Light 时新(亮)快照从点击点**扩张**盖住旧(暗)。③ 两快照默认 `animation:none`,动态样式只给其一挂 clip 动画(z-index 决定谁在上)。**关键——分数缩放偏移根因与修法**:Chrome 在非整数 `devicePixelRatio`(Windows DPI 166% → dpr 1.666 等)下给 `::view-transition-group(root)` 套 fractional `transform` 做设备像素→CSS px 重投影,会把整层(含 clip-path 圆)推偏;修法是 `::view-transition-group(root){transform:none!important;inset:0;width:100vw;height:100vh}` **中和该 transform**、并把 old/new 钉在视口(`inset:0;width:100%;height:100%`),让点击坐标与伪元素坐标系一致(协作者 `f4d3442` 根治)。历史:`85d22f2` 曾为此弃 VT、`b5be430` 为"圆内外真实内容"换回、`78b939d` 退化为交叉淡入、`d801f32` 又恢复圆扩散。⚠️ `captureVisibleTab` 截旧画面方案因 MV3 需 `activeTab`/`<all_urls>`、内容脚本点击不授权而弃用;`ThemeReveal.vue` 仅作无 VT 浏览器回退。
