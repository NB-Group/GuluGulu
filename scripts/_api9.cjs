    ...
    return new Promise(function(resolve){
      ...
      ws.onopen = function(){
        ...
        xhr.onload = function(){
          ...
          } catch(e){ all.push({t:"parse_err",v:e.message}); }
        };
      ...
      ws.onmessage = function(e){
        try {
          ...
          all.push({t:"recv",ws_type:m._ws_type,param:m._channel_param,full:JSON.stringify(m).slice(0,300)});
          if (m._ws_type === "server_broadcast") { clearTimeout(t); ws.close(); resolve({ok:true,output:m.output,all:all}); }
        } catch(ex){ all.push({t:"raw",v:e.data.slice(0,100)}); }
      };
      ws.onerror = function(){ all.push({t:"err"}); };
      ws.onclose = function(e){ all.push({t:"close",code:e.code}); };
    });
  }, {csrf:csrf});
