...
export function useDark() {
  ...
  // Add the 'dark' class to #gulu and <html> so Unocss dark-specific styles apply.
  watch(
  ...
  // 下用 html.gulu-vt-clip 类启用;分数 dpr 时不下发该类 → VT 走默认交叉淡入(见 toggleDark)。
  let vtStyle: HTMLStyleElement | null = null
  function ensureViewTransitionCss() {
    if (vtStyle)
      return
    vtStyle = document.createElement('style')
    vtStyle.textContent = 'html.gulu-vt-clip ::view-transition-old(root),html.gulu-vt-clip ::view-transition-new(root){animation:none!important;mix-blend-mode:normal}html.gulu-vt-clip ::view-transition-old(root){z-index:1}html.gulu-vt-clip ::view-transition-new(root){z-index:2}'
  ...
  async function toggleDark(e?: MouseEvent) {
    ...
    document.documentElement.classList.add('gulu-vt-clip')
    ...
    t.ready.then(() => {
      ...
      try {
        ...
        if (Math.abs(dpr - Math.round(dpr)) > 0.01) {
          ...
          console.debug('[gulu-vt-offset]', JSON.stringify({ dpr, vw: innerWidth, vh: innerHeight, click: { x, y }, group: { top: g.top, left: g.left, w: g.width, h: g.height }, new: { top: n.top, left: n.left, w: n.width, h: n.height } }))
        }
      }
      catch {}
