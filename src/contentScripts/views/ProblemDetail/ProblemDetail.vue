...
const contestId = computed(() => {
  const m = (currentUrl.value || window.location.href).match(/[?&]contestId=(\d+)/)
  return m ? m[1] : ''
})
...
if (typeof window !== 'undefined' && window.matchMedia) {
  ...
  if (typeof mq.addEventListener === 'function')
    ...
    (mq as any).addListener(sync)
  onUnmounted(() => {
    if (typeof mq.removeEventListener === 'function')
      mq.removeEventListener('change', sync)
    else if (typeof (mq as any).removeListener === 'function')
      (mq as any).removeListener(sync)
  })
}
...
<template>
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
          ...
          <div v-if="!isSplitView" key="normal">
            ...
            >
              ...
              <div class="hidden md:flex items-center gap-3" px-3 text="xs $bew-text-3" style="margin-left:auto">
                ...
                <button
                  flex="~ items-center gap-1" cursor="pointer" p="x-2.5 y-1" rounded="$bew-radius-half" text="sm"
              ...
              <button
                v-if="!isSplitView" class="ml-auto" flex="~ items-center gap-1" p="x-3 y-2" rounded="$bew-radius-half"
                text="sm"
                border="none"
                :disabled="!isLoggedIn"
            ...
            <Transition v-if="!isSplitView" name="page-fade" mode="out-in">
              ...
              <!-- ============================================================ -->
              <SolutionsTab
                v-else-if="activeTab === 'solutions'" key="solutions" :solutions="solutions" :solutions-loading="solutionsLoading" :need-login="solutionsNeedLogin"
    ...
    <Dialog
      v-if="myRecordsVisible" v-model:visible="myRecordsVisible" title="我的提交记录 · 本题" :show-footer="false" append-to-body
      :width="520"
    >
