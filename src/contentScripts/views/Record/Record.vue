...
function tcStatus(tc: any): { label: string, color: string } {
  ...
  if (!Number.isNaN(st) && tcStatusMap[st])
...
async function fetchRecords(append = false) {
  ...
  errorMsg.value = ''
  try {
    const res = await fetch(`https://www.luogu.com.cn/record/list?_contentOnly=1&page=${listPage.value}`, {
...
const recordId = computed(() => {
  const m = currentUrl.value.match(/\/record\/(\d+)/i)
...
function hideTcTooltip() {
  tcHideTimer = setTimeout(() => {
    hoveredTc.value = null
  }, 150)
}

function openRecord(rid: number) {
  navigateTo(AppPage.Record, `https://www.luogu.com.cn/record/${rid}`)
}
function backToList() {
  navigateTo(AppPage.Record, 'https://www.luogu.com.cn/record/list')
}
...
<template>
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
      ...
      <Transition name="content-reveal">
        ...
        >
          ...
          <div flex="~ items-center gap-4" mb-4 pb-4 border="b-1 $bew-border-color">
            ...
            <div
              ...
              :style="{
                ...
                fontWeight: 600,
                color:
                  detail.status === 12 || detail.status === 4 ? 'var(--bew-success-color)' : 'var(--bew-error-color)',
              }"
            >
              {{ detail.score }} 分
            </div>
          ...
          <div grid="~ cols-2 md:cols-4" gap-4 mb-6 style="font-size: var(--bew-base-font-size)">
            <div>
              <span style="color: var(--bew-text-3)">题目</span><br><span
                style="color: var(--bew-theme-color); cursor: pointer; font-weight: 500"
                @click="openProblem(detail.problem.pid)"
              >{{ detail.problem.pid }} {{ detail.problem.title }}</span>
            </div>
            <div>
              <span style="color: var(--bew-text-3)">用时</span><br><span style="color: var(--bew-text-1)">{{
                formatTime(detail.time || 0)
              }}</span>
            </div>
            <div>
              <span style="color: var(--bew-text-3)">内存</span><br><span style="color: var(--bew-text-1)">{{
                formatMemory(detail.memory || 0)
              }}</span>
            </div>
            <div>
              <span style="color: var(--bew-text-3)">语言</span><br><span style="color: var(--bew-text-1)">{{ langName(detail.language) }} {{ detail.enableO2 ? '(O2)' : '' }}</span>
            </div>
            <div>
              <span style="color: var(--bew-text-3)">代码长度</span><br><span style="color: var(--bew-text-1)">{{ detail.sourceCodeLength }} B</span>
            </div>
            <div>
              <span style="color: var(--bew-text-3)">提交时间</span><br><span style="color: var(--bew-text-1)">{{
                new Date(detail.submitTime * 1000).toLocaleString('zh-CN')
              }}</span>
            </div>
          </div>
          ...
          <div
            v-if="detail.detail?.compileResult?.message"
            bg="$bew-fill-1"
            rounded="$bew-radius"
            p-4
            mb-6
          ...
          <template v-if="testCaseGroups.length > 0">
            <div v-for="(sub, si) in testCaseGroups" :key="si" mb-6>
              <div
                v-if="testCaseGroups.length > 1"
                style="font-size: 0.85em; color: var(--bew-text-2); font-weight: 600"
                mb-2
              >
                Subtask #{{ sub.id }}
                {{ sub.fullScore != null ? `(${sub.score ?? 0}/${sub.fullScore} 分)` : `(${sub.score ?? 0} 分)` }}
              </div>
              <div flex="~ wrap" gap-2>
                <div
                  v-for="(tc, idx) in sub.cases"
                  :key="idx"
                  class="tc-block"
          ...
          <div v-if="detail.sourceCode" mt-6>
            ...
            <pre
              ...
              style="
                ...
                tab-size: 4;
              "
              v-html="highlightCode(detail.sourceCode, String(detail.language))"
            />
          </div>
        </div>
      </Transition>
    ...
    <template v-else>
      ...
      <Transition name="content-reveal">
        ...
        >
          <div
            ...
            cursor="pointer"
            duration-200
            @click="openRecord(r.rid)"
          >
            <div w="80px" flex-shrink-0>
              <span :style="{ color: statusColor(r.status), fontSize: 'var(--bew-base-font-size)', fontWeight: 600 }">{{
            ...
            <div flex="1" min-w-0>
              <div
                ...
                @click.stop="openProblem(r.problem.pid)"
              >
              ...
              <div
                flex="~ gap-3"
                mt-1
                style="font-size: calc(var(--bew-base-font-size) * 0.85); color: var(--bew-text-3)"
              >
                <span>#{{ r.rid }}</span>
            ...
            <div
              ...
              flex-shrink-0
              text-right
              style="font-size: var(--bew-base-font-size); font-weight: 600"
              :style="{ color: r.score === 100 ? 'var(--bew-success-color)' : 'var(--bew-error-color)' }"
            >
              {{ r.score }} 分
            </div>
          </div>
      ...
      <div
        v-if="!loading && records.length > 0 && listPage < totalPages"
        ref="sentinelRef"
        style="
          height: 60px;
          display: flex;
      ...
      <div
        v-if="!loading && records.length > 0 && listPage >= totalPages"
        style="font-size: var(--bew-base-font-size); color: var(--bew-text-3); text-align: center; padding-bottom: 2rem"
      >
