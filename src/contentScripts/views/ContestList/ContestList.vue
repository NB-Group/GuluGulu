...
function getContestStatus(c: any): { label: string, color: string } {
  ...
  if (c.startTime && now < c.startTime)
    return { label: '即将开始', color: 'var(--bew-success-color)' }
  if (c.endTime && now < c.endTime)
    return { label: '进行中', color: 'var(--bew-error-color)' }
    ...
    <Transition name="content-reveal">
      <div v-if="!loading && contests.length > 0" grid="~ cols-1 md:cols-2 xl:cols-3" gap-4 mb-6>
        <div
          ...
          :key="c.id"
          class="stagger-card contest-card"
        ...
        >
          <div flex="~ items-center justify-between" mb-3>
            <span
              :style="{
                background: `${getContestStatus(c).color}20`,
                color: getContestStatus(c).color,
                fontSize: 'var(--bew-base-font-size)',
                fontWeight: 600,
                padding: '2px 10px',
                borderRadius: '9999px',
              }"
            >{{ getContestStatus(c).label }}</span>
            <span
              v-if="c.rated"
              style="font-size: var(--bew-base-font-size); color: var(--bew-success-color); font-weight: 600"
            >Rated</span>
          </div>
