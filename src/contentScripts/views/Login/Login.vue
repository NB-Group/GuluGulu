  ...
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
    ...
    >
      <template v-if="isLoggedIn">
        <span
          style="display: contents; color: var(--bew-success-color)"
          v-html="renderIcon('mingcute:check-circle-fill', 56)"
        />
        <h1 style="font-size: 1.25rem; color: var(--bew-text-1); font-weight: 700" mt-4 mb-2>
          已登录洛谷
        </h1>
        ...
        <div flex="~ gap-3" mt-6>
          <Button type="secondary" @click="refreshPage">
            <span style="display: contents" v-html="renderIcon('mingcute:refresh-line', 16)" /> 刷新状态
          </Button>
          <Button type="primary" @click="doLogin">
            <span style="display: contents" v-html="renderIcon('mingcute:user-4-line', 16)" /> 切换账号
          </Button>
        </div>
      </template>

      <template v-else>
        <span style="display: contents; color: var(--bew-text-3)" v-html="renderIcon('mingcute:user-4-line', 48)" />
        <h1 style="font-size: 1.25rem; color: var(--bew-text-1); font-weight: 700" mt-4 mb-2>
          登录洛谷
        </h1>
        ...
        <Button type="primary" block center @click="doLogin">
          <span style="display: contents" v-html="renderIcon('mingcute:arrow-right-line', 16)" />
          前往洛谷登录
        </Button>
