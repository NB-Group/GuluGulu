  ...
  <div :class="{ 'page-container': !props.embedded }" w-full h-full :p="props.embedded ? '' : 'x-4 md:x-8 lg:x-16'">
    ...
    <Transition name="content-reveal">
      <div v-if="!loading && items.length>0" bg="$bew-content" rounded="$bew-radius" shadow="[var(--bew-shadow-1)]" border="1 $bew-border-color" overflow="hidden">
        <div v-for="(n, idx) in items" :key="n.id || idx" class="stagger-row hover:bg-$bew-fill-2" :style="{'--row-index':idx}" p="x-6 y-3.5" border="b-1 $bew-border-color">
          <div style="font-size:var(--bew-base-font-size);color:var(--bew-text-1)" v-html="n.content || n.title || n.text" />
          <div style="font-size:.8em;color:var(--bew-text-3)" mt-1>{{ timeAgo(n.time || 0) }}</div>
        </div>
      </div>
    </Transition>
    <div v-if="!loading && items.length===0" bg="$bew-content" rounded="$bew-radius" p-8 text="center $bew-text-3" border="1 $bew-border-color">
      <span v-html="renderIcon('mingcute:notification-line',48)" style="display:contents"/><p mt-2>暂无通知</p>
    </div>
  </div>
</template>
