...
<template>
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
    ...
    <Transition name="content-reveal">
      ...
      >
        <div
          ...
          fw-bold
        >
          <div w="60px" text-center>
            #
          </div>
          <div flex="1">
            用户
          </div>
          <div w="80px" text-center>
            咕值
          </div>
        </div>
        <div
        ...
        >
          <div w="60px" flex="~" items="center" justify="center">
            <span
              v-if="idx < 3"
              :style="{ color: ['#B8860B', '#808080', '#8B4513'][idx] }"
              style="display: contents"
              v-html="renderIcon('mingcute:medal-fill', 22)"
            />
          ...
          <div flex="~ 1" items="center" gap-3>
            ...
            >
              <img
                ...
                "
              >
              <span style="pointer-events: none" v-html="renderIcon('mingcute:user-4-line', 18)" />
            </div>
            <div flex="~ col">
              ...
              <span
                ...
                text="xs $bew-text-3"
                overflow-hidden
                style="text-overflow: ellipsis; white-space: nowrap; max-width: 200px"
              >{{ u.slogan }}</span>
            </div>
          </div>
          <div w="80px" text="sm" fw-bold text-center :style="{ color: ratingColor(u.rating) }">
            {{ u.rating }}
          </div>
        </div>
      </div>
    ...
    <div
      v-if="!loading && users.length > 0 && currentPage < totalPages"
      ref="sentinelRef"
    ...
    <div v-if="!loading && users.length > 0 && currentPage >= totalPages" text="sm $bew-text-3" text-center pb-8>
      已加载全部 {{ totalCount }} 名用户
    </div>
  </div>
</template>
