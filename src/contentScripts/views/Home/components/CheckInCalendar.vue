...
const checkInState
  = (window as any).__guly_checkin || ((window as any).__guly_checkin = { date: '', done: false, fortune: null as any })
...
function saveCheckInState() {
  checkInState.date = todayStr()
  checkInState.done = checkInDone.value
  checkInState.fortune = fortuneResult.value
}
...
function parsePunchHtml(html: string) {
  ...
  for (const m of html.matchAll(/宜：<[^>]*>([^<]+)<[^>]*>[\s\S]*?lg-small[^>]*>([^<]+)</g))
    r.good.push(`${m[1].trim()} — ${m[2].trim()}`)
  for (const m of html.matchAll(/忌：<[^>]*>([^<]+)<[^>]*>[\s\S]*?lg-small[^>]*>([^<]+)</g))
    r.bad.push(`${m[1].trim()} — ${m[2].trim()}`)
  ...
  >
    ...
    <div class="calendar-header" text="center" mb-2 w-full>
      ...
      <div class="calendar-date-display" flex="~ col items-center" gap-1>
        <span text="sm" fw-bold style="color: var(--bew-theme-color)"> {{ monthName }}{{ monthSizeLabel }} </span>
    ...
    <div class="countdown-section" w-full text="xs $bew-text-2" flex="~ col" gap-1>
      <p v-if="cspRound1Days > 0" text="center">
        距 <strong style="color: var(--bew-theme-color)">CSP-J/S 2026 第一轮</strong> 还剩
        <strong text="$bew-text-1">{{ cspRound1Days }} 天</strong>
      </p>
      <p v-if="cspRound2Days > 0" text="center">
        距 <strong style="color: var(--bew-theme-color)">CSP-J/S 2026 第二轮</strong> 还剩
        <strong text="$bew-text-1">{{ cspRound2Days }} 天</strong>
      </p>
    </div>
  </div>
</template>
