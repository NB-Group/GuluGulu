...
function sLabel(s: number): string {
  if (s === 1)
    return '进行中'
  if (s === 2)
    return '已结束'
...
function openContest(id: number) {
  window.open(`https://www.luogu.com.cn/contest/${id}`, '_blank')
}
    ...
    >
      ...
      <Transition name="content-reveal">
        <div v-if="!loading" flex="~ col" gap-3>
          <div
            v-for="(c, idx) in contests"
            :key="c.id"
            class="stagger-row contest-row"
            :style="{ '--row-index': idx }"
            bg="$bew-fill-1"
            rounded="8px"
            p-3
          ...
          >
            <div flex="~ items-center gap-2" flex-wrap mb-2>
              <span
                ...
                py-0.5
                rounded-full
                :style="{ background: `${sColor(c.status)}20`, color: sColor(c.status) }"
              >{{ sLabel(c.status) }}</span>
              <span
                style="font-size: 0.75em; color: var(--bew-theme-color)"
                fw-bold
                px-2
                py-0.5
                rounded-full
              ...
              <span
                v-if="c.rated"
                style="font-size: 0.75em; background: var(--bew-warning-color-20); color: var(--bew-warning-color)"
                fw-bold
                px-2
                py-0.5
                rounded-full
            ...
            <h3 style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); font-weight: 600" mb-1>
              {{ c.name }}
            </h3>
