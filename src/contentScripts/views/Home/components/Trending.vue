    ...
    if (cm?.[1]) {
      ...
      for (const c of contests.slice(0, 5)) {
        ...
        list.push({
          ...
          time: c.startTime || 0,
          detail: (c.rated && c.rated > 0 ? 'Rated' : '') + (isEnded ? ' 已结束' : ''),
      ...
      for (const p of posts.slice(0, 8)) {
        if (!p.topped) {
          list.push({
            ...
            title: p.title || '',
            time: p.time || 0,
            detail: `${p.replyCount || 0} 回复`,
            url: `https://www.luogu.com.cn/discuss/${p.id}`,
          })
        }
      }
    }
...
function openItem(url: string) {
  if (url)
    window.open(url, url.includes('/discuss/') ? '_self' : '_blank')
}
    ...
    >
      ...
      <Transition name="content-reveal">
        <div v-if="!loading" flex="~ col" gap-2>
          <div
            v-for="(act, idx) in items"
            :key="act.id"
            class="stagger-row trend-row"
            :style="{ '--row-index': idx }"
            ...
            cursor-pointer
            duration-200
            @click="openItem(act.url)"
          >
            <span
              :style="{ color: typeColor(act.type), display: 'contents', flexShrink: 0 }"
              v-html="renderIcon(typeIcon(act.type), 18)"
            />
            <div flex="1" min-w-0>
              <div
                style="
              ...
              <div flex="~ items-center gap-2" style="font-size: 0.75em; color: var(--bew-text-3)">
