  ...
  try {
    ...
    if (m?.[1]) {
      ...
      problems.value = list.slice(0, 12).map((p: any) => ({
        ...
        accepted: p.totalAccepted || p.acceptedCount || 0,
        submitted: p.totalSubmit || p.submittedCount || 1,
      }))
    }
  }
  catch {}
...
function passRate(a: number, s: number) {
  ...
  <div>
    ...
    >
      <div mb-4>
        ...
        <p style="font-size: 0.85em; color: var(--bew-text-3)" mt-1>
          洛谷题库
        </p>
      </div>
      ...
      <Transition name="content-reveal">
        <div v-if="!loading" grid="~ cols-1 md:cols-2 lg:cols-3" gap-4>
          <div
            ...
            :key="p.pid"
            class="stagger-row problem-card"
            :style="{ '--row-index': idx }"
            bg="$bew-fill-1"
            rounded="8px"
            p-3
          ...
          >
            <div flex="~ items-center justify-between" mb-2>
              <span style="font-size: 0.85em; color: var(--bew-text-3)" font-mono>{{ p.pid }}</span>
              <span
            ...
            <h3 style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); font-weight: 600" mb-2>
              {{ p.title }}
            </h3>
            <div flex="~ items-center gap-1" style="font-size: 0.8em; color: var(--bew-text-3)">
              <span style="display: contents" v-html="renderIcon('mingcute:check-circle-line', 14)" /> 通过率
              {{ passRate(p.accepted, p.submitted) }}%
            </div>
          </div>
          <div
            v-if="problems.length === 0"
            style="text-align: center; color: var(--bew-text-3); font-size: var(--bew-base-font-size)"
            py-8
          >
            获取题目失败
          </div>
