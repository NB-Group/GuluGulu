...
interface Post {
  ...
  locked: boolean
  replyCount: number
  forum: { name: string, color: string }
          ...
          >
            ...
            <div flex="1" min-w-0>
              ...
              <div flex="~ items-center gap-2" style="font-size: 0.75em; color: var(--bew-text-3)">
                <span
                  :style="{
                    color: p.author.color ? `var(--bew-${p.author.color})` : 'var(--bew-text-2)',
                    fontWeight: 600,
                  }"
                >{{ p.author.name }}</span>
                <span>{{ timeAgo(p.time) }}</span>
                <span flex="~ items-center gap-1"><span style="display: contents" v-html="renderIcon('mingcute:comment-line', 12)" />{{
                  p.replyCount
                }}</span>
              </div>
            </div>
            <span
              style="font-size: 0.7em; color: var(--bew-text-3)"
              px-1.5
              py-0.5
              rounded-full
              bg="$bew-fill-2"
