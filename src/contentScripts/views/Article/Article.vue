    ...
    <template v-if="isLoggedIn && articleLid">
      ...
      >
        ...
        <div v-if="detail" flex="~ items-center gap-3 wrap" mt-2 style="font-size:.9em;color:var(--bew-text-3)">
          ...
          <span :style="{ color: detail.author?.color ? `var(--bew-${detail.author.color})` : 'var(--bew-text-1)' }" fw-bold>{{ detail.author?.name }}<UserBadges :uid="detail.author?.uid" :name="detail.author?.name" :luogu-badge="detail.author?.badge" :color="detail.author?.color ? `var(--bew-${detail.author.color})` : undefined" /></span>
          <span
