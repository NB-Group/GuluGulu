...
const trainingId = computed(() => {
  const m = document.URL.match(/\/training\/(\d+)/i)
  ...
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
    ...
    <template v-if="trainingId">
      ...
      >
        ...
        <div v-if="detailMeta" mt-2 flex="~ gap-4" style="font-size: 0.85em; color: var(--bew-text-3)">
      ...
      <Transition name="content-reveal">
        <div
          v-if="!detailLoading && detailProblems.length > 0"
          bg="$bew-content"
          rounded="$bew-radius"
          shadow="[var(--bew-shadow-1),var(--bew-shadow-edge-glow-1)]"
          border="1 $bew-border-color"
        ...
        >
          ...
          >
            ...
            <span
              ...
              mr-3
              flex-shrink-0
              style="font-size: 0.7em; font-weight: 600; white-space: nowrap"
              :style="{ color: diffColor(p.difficulty), background: `${diffColor(p.difficulty)}20` }"
            ...
            >{{ p.title }}</span>
            <!-- Pass rate -->
            <span flex-shrink-0 mx-3 style="font-size: 0.8em; color: var(--bew-text-3); white-space: nowrap">{{
              passRate(p.totalAccepted, p.totalSubmit)
            }}</span>
            <!-- Status -->
            <span
              v-if="p.accepted"
              flex-shrink-0
              style="color: var(--bew-success-color); font-size: var(--bew-base-font-size)"
              flex="~ items-center gap-1"
            ><span style="display: contents" v-html="renderIcon('mingcute:check-circle-fill', 16)" /> AC</span>
            <span
              v-else-if="p.submitted"
              flex-shrink-0
              style="color: var(--bew-error-color); font-size: var(--bew-base-font-size)"
            >WA</span>
    ...
    <template v-else>
      <div
        ...
        style="backdrop-filter: var(--bew-filter-glass-1)"
      >
        <h1 style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); font-weight: 700" mb-4>
          题单
        </h1>
        <p style="color: var(--bew-text-2); font-size: var(--bew-base-font-size)">
          共 {{ trainings.length }} 个题单
        </p>
      </div>
      ...
      <Transition name="content-reveal">
        <div v-if="!loading && trainings.length > 0" grid="~ cols-1 md:cols-2 xl:cols-3" gap-4 mb-6>
          <div
            ...
            :key="t.id"
            class="stagger-card training-card"
          ...
          >
            ...
            <div flex="~ items-center gap-4" style="font-size: var(--bew-base-font-size); color: var(--bew-text-2)">
              <span flex="~ items-center gap-1"><span style="display: contents" v-html="renderIcon('mingcute:document-line', 14)" />{{
                t.problemCount
              }}
                题</span>
              <span v-if="t.acCount" flex="~ items-center gap-1" style="color: var(--bew-success-color)"><span style="display: contents" v-html="renderIcon('mingcute:check-circle-line', 14)" />{{
                t.acCount
              }}
                已完成</span>
            </div>
...
.training-problem-row {
  transition: background 0.15s;
}
.training-problem-row:hover {
