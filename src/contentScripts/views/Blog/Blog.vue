    ...
    <template v-if="isLoggedIn && discussId">
      <div bg="$bew-content" rounded="$bew-radius" p-6 mb-6 shadow="[var(--bew-shadow-1),var(--bew-shadow-edge-glow-1)]" border="1 $bew-border-color" style="backdrop-filter:var(--bew-filter-glass-1)">
        ...
        <div v-if="detail" flex="~ items-center gap-3" mt-2 style="font-size:var(--bew-base-font-size);color:var(--bew-text-3)">
          ...
          <span :style="{color:detail.author?.color?`var(--bew-${detail.author.color})`:'var(--bew-text-1)'}">{{ detail.author?.name }}<UserBadges :uid="detail.author?.uid" :name="detail.author?.name" :luogu-badge="detail.author?.badge" :color="detail.author?.color ? `var(--bew-${detail.author.color})` : undefined" /></span>
