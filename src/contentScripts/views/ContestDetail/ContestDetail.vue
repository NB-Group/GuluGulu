...
import { friendlyError, getCsrfToken, LUOGU_LANGUAGES } from '~/utils/luogu-api'
...
interface ScoreboardRow {
  rank: number
  user: { uid: number, name: string, avatar: string, color: string }
  score: number
  scores: (number | null)[]
  totalTime?: number
}
  ...
  try {
    ...
    if (m?.[1]) {
      ...
      if (contest.value) {
        ...
        userRegistration.value = cd.joined
          ? { registered: true }
          : contest.value.userRegistration || cd.userRegistration || cd.registration || null
        const host = cd?.host || contest.value.host
        if (host)
          contest.value.host = host
        const cert = cd?.description || contest.value.description
        if (cert)
          contest.value.description = cert
        const tcr = cd?.totalRegisteredUsers || cd?.registeredCount || contest.value.totalRegisteredUsers
        if (tcr)
          contest.value.totalRegisteredUsers = tcr
        if (!contest.value.ruleType)
          contest.value.ruleType = cd?.ruleType || 1
      }
    }
    else {
...
async function handleRegister() {
  ...
  try {
    ...
    regMsg.value = `报名失败：${e.message || '网络错误'}`
  }
...
async function loadProblem(pid: string) {
  ...
  try {
    const res = await fetch(`https://www.luogu.com.cn/problem/${pid}?contestId=${contestId.value}`, {
      credentials: 'same-origin',
    })
    const html = await res.text()
    const m = html.match(/<script\s+id="lentille-context"\s+type="application\/json"[^>]*>([^<]+)<\/script>/)
...
async function handleSubmit() {
  if (!selectedPid.value) {
    submitError.value = '请先选择题目'
    return
  }
  if (!code.value.trim()) {
    submitError.value = '请先输入代码'
    return
  }
  ...
  try {
    ...
    submitError.value = `提交失败：${e.message || '网络错误'}`
  }
...
async function fetchScoreboard(page = 1) {
  ...
  try {
    const res = await fetch(`https://www.luogu.com.cn/fe/api/contest/scoreboard/${contestId.value}?page=${page}`, {
      credentials: 'same-origin',
    })
    const json = await res.json()
    ...
    if (data?.scoreboard?.result) {
      scoreboard.value = data.scoreboard.result.map((r: any) => ({
        rank: r.rank || r.idx,
        user: {
          uid: r.user?.uid || r.uid,
          name: r.user?.name || r.name || '',
          avatar: r.user?.avatar || `https://cdn.luogu.com.cn/upload/usericon/${r.user?.uid || r.uid}.png`,
          color: r.user?.color || r.color || '',
        },
...
function countdown(): string {
  const now = nowTs.value
  const start = c.value.startTime || 0
  const end = c.value.endTime || 0
  if (now < start) {
    const d = start - now
    if (d >= 86400)
      return `距开始 ${Math.floor(d / 86400)} 天 ${Math.floor((d % 86400) / 3600)} 小时`
    const h = Math.floor(d / 3600)
    const m = Math.floor((d % 3600) / 60)
  ...
  if (now < end) {
    const d = end - now
    if (d >= 86400)
      return `距结束 ${Math.floor(d / 86400)} 天 ${Math.floor((d % 86400) / 3600)} 小时`
    const h = Math.floor(d / 3600)
    const m = Math.floor((d % 3600) / 60)
...
function problemLabel(idx: number) {
...
function openOriginal() {
  window.open(`https://www.luogu.com.cn/contest/${contestId.value}`, '_blank')
}
function openRecord(rid: number) {
  window.open(`https://www.luogu.com.cn/record/${rid}`, '_blank')
}
...
watch(activeTab, (t) => {
  if (t === 'ranking' && scoreboard.value.length === 0)
    fetchScoreboard()
})
</script>
      ...
      <div v-if="!loading && contest" w-full>
          ...
          <div flex="~ col gap-4">
            <div flex="~ items-center gap-2" flex-wrap>
              <span
                ...
                py-1
                rounded-full
                :style="{ background: `${contestStatus.color}20`, color: contestStatus.color }"
              >{{ contestStatus.label }}</span>
              <span
                text="xs"
                fw-bold
                px-3
              ...
              <span
                ...
                py-1
                rounded-full
                style="background: var(--bew-warning-color-20); color: var(--bew-warning-color)"
              >Rated</span>
              <template v-if="countdown()">
                <span
                  ...
                  py-1
                  rounded-full
                  style="background: var(--bew-error-color-20); color: var(--bew-error-color)"
                >{{ countdown() }}</span>
              </template>
            </div>
            ...
            <div
              ...
              mb-1
            >
              <span style="font-size: 0.85em">主办:</span>
              <img
                :src="c.host.avatar"
            ...
            <div v-if="registeredCount > 0" style="font-size: var(--bew-base-font-size); color: var(--bew-text-3)">
              {{ registeredCount }} 人已报名
            </div>
            <!-- Registration button -->
            <div v-if="!contestStatus.ended" flex="~ items-center gap-3">
              <Button v-if="!isRegistered" type="primary" :loading="regLoading" @click="handleRegister">
                <span style="display: contents" v-html="renderIcon('mingcute:user-add-line', 16)" /> 报名参赛
              </Button>
              <span
                v-else
                px-3
                ...
                "
              >已报名</span>
              <span v-if="regMsg" style="font-size: var(--bew-base-font-size); color: var(--bew-success-color)">{{
                regMsg
              }}</span>
            </div>
            <div v-if="contestStatus.ended" style="font-size: var(--bew-base-font-size); color: var(--bew-text-3)">
              比赛已结束
            </div>
            <div
              v-if="!canAccessContent && isRegistered"
              mt-2
              p-3
              rounded="$bew-radius"
              style="
                background: var(--bew-warning-color-20);
        ...
        >
          <div flex="~" border="b-1 $bew-border-color">
            <template v-for="tab in ['overview', 'problems', 'submit', 'ranking'] as const" :key="tab">
              <button
                v-if="tab === 'overview' || canAccessContent"
                flex="1"
                style="
                  background: none;
                  border: none;
                ...
                "
                :style="{
                  color: activeTab === tab ? 'var(--bew-theme-color)' : 'var(--bew-text-3)',
                  borderBottom: activeTab === tab ? '2px solid var(--bew-theme-color)' : '2px solid transparent',
                }"
                @click="activeTab = tab"
              >
        ...
        >
          <div
            v-if="c.description"
            class="markdown-body"
            style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); line-height: 1.8"
            v-html="parseMarkdownContent(c.description)"
          />
          <div
            v-else
            style="text-align: center; padding: 40px; color: var(--bew-text-3); font-size: var(--bew-base-font-size)"
          >
            <span style="display: contents" v-html="renderIcon('mingcute:info-line', 48)" />
            <p mt-2>
              暂无比赛介绍
            </p>
          </div>
        ...
        >
          <a
            ...
            :key="p.pid"
            :href="`https://www.luogu.com.cn/problem/${p.pid}?contestId=${contestId}#ide`"
            target="_blank"
            class="stagger-row hover-row"
            :style="{ '--row-index': idx, 'textDecoration': 'none', 'color': 'inherit' }"
            flex="~ items-center"
            px-6
            py-4
          ...
          >
            <span
              style="
                ...
                flex-shrink: 0;
              "
            >{{ problemLabel(idx) }}</span>
            <div flex="1" min-w-0 mx-3>
              <div style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); font-weight: 600">
                {{ p.pid }} {{ p.title }}
              </div>
              <div v-if="p.submitted != null && p.submitted >= 0" style="font-size: 0.8em; color: var(--bew-text-3)">
                通过 {{ p.accepted }} / 提交 {{ p.submitted }}
              </div>
            </div>
            <span
              style="
            ...
            <button
              ...
              style="
                ...
                font-weight: 600;
              "
              @click.prevent="((activeTab = 'submit'), (selectedPid = p.pid), loadProblem(p.pid))"
            >
              提交
            </button>
          </a>
          ...
          >
            ...
            <div mb-4>
              ...
              <select
                ...
                style="
                  ...
                  font-size: var(--bew-base-font-size);
                "
                @change="loadProblem(selectedPid)"
              >
                <option value="">
                  -- 请选择题目 --
                </option>
                <option v-for="(p, idx) in problems" :key="p.pid" :value="p.pid">
                  {{ problemLabel(idx) }}. {{ p.title }} ({{ p.score }}分)
                </option>
              </select>
            </div>

            <!-- Problem statement (condensed) -->
            <div v-if="statementLoading" p-4 text="center">
              <Loading />
            </div>
            <div
              v-if="!statementLoading && problemStatement"
              bg="$bew-fill-1"
              rounded="$bew-radius"
              p-4
              mb-4
              ...
              "
            >
              <div fw-bold mb-2 style="color: var(--bew-text-1)">
                {{ problemStatement.pid }} {{ problemStatement.title }}
              </div>
              <div
                style="white-space: pre-wrap"
                v-text="
                  `${problemStatement.background || ''} ${(problemStatement.description || '').replace(/<[^>]+>/g, '').slice(0, 800)}${problemStatement.description?.length > 800 ? '...' : ''}`
                "
              />
            </div>
            ...
            <div flex="~ items-center gap-3" mb-3>
              <select
                ...
                "
              >
                <option v-for="l in LUOGU_LANGUAGES.slice(0, 12)" :key="l.id" :value="l.id">
                  {{ l.name }}
                </option>
              </select>
              <label
                flex="~ items-center gap-1"
                style="font-size: var(--bew-base-font-size); color: var(--bew-text-2); cursor: pointer"
              >
                <input v-model="enableO2" type="checkbox"> O2优化
              </label>
            </div>
            ...
            >
              提交成功！
              <span
                v-if="submitResult.rid"
                style="color: var(--bew-theme-color); cursor: pointer; font-weight: 600"
                @click="openRecord(submitResult.rid)"
              >查看记录 #{{ submitResult.rid }}</span>
            </div>
            <Button
              type="primary" mt-3 w-full :loading="submitLoading" :disabled="!selectedPid"
              @click="handleSubmit"
            >
              <span style="display: contents" v-html="renderIcon('mingcute:send-line', 16)" /> 提交代码
            </Button>
          </div>
        </div>
        ...
        >
          ...
          <div
            v-if="!rankingLoading && scoreboard.length === 0"
            text="center"
            p-8
            style="color: var(--bew-text-3); font-size: var(--bew-base-font-size)"
          >
            <span style="display: contents" v-html="renderIcon('mingcute:chart-bar-line', 48)" />
            <p mt-2>
              暂无排名数据{{ contestStatus.upcoming ? '（比赛尚未开始）' : '' }}
            </p>
          </div>
          <!-- Scoreboard table -->
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: var(--bew-base-font-size)">
              <thead>
                <tr bg="$bew-fill-1" text="left">
                  <th px-4 py-3 style="color: var(--bew-text-2); font-weight: 600">
                    #
                  </th>
                  <th px-4 py-3 style="color: var(--bew-text-2); font-weight: 600">
                    选手
                  ...
                  <th
                    v-for="(_p, pi) in problems"
                    :key="pi"
                    px-4
                    py-3
                    text="center"
                    style="color: var(--bew-text-2); font-weight: 600"
                  >
                    {{ problemLabel(pi) }}
                  </th>
                  <th px-4 py-3 text="right" style="color: var(--bew-text-2); font-weight: 600">
                    总分
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  duration-150
                  cursor="pointer"
                  @click="openUser(row.user.uid)"
                >
                  <td
                    px-4
                    py-3
                    fw-bold
                    :style="{ color: row.rank <= 3 ? 'var(--bew-warning-color)' : 'var(--bew-text-1)' }"
                  >
                    {{ row.rank }}
                  </td>
                  <td px-4 py-3 flex="~ items-center gap-2">
                    ...
                    <span
                      :style="{
                        color: row.user.color ? `var(--bew-${row.user.color})` : 'var(--bew-text-1)',
                        fontWeight: 600,
                      }"
                    >{{ row.user.name }}</span>
                  </td>
                  <td
                    v-for="(s, si) in row.scores"
                    :key="si"
                    px-4
                    py-3
                    text="center"
                    ...
                    :style="{
                      color:
                        s != null && s >= (problems[si]?.score || 100)
                          ? 'var(--bew-success-color)'
                          : s != null
                            ? 'var(--bew-error-color)'
                            : 'var(--bew-text-4)',
                    }"
                  ...
                  <td px-4 py-3 text="right" fw-bold style="color: var(--bew-text-1)">
                    {{ row.score }}
                  </td>
                </tr>
              </tbody>
          ...
          <div v-if="rankingTotal > scoreboard.length" p-4 text="center">
            <Button type="secondary" @click="(rankingPage++, fetchScoreboard(rankingPage))">
              加载更多排名
            </Button>
          </div>
        </div>
