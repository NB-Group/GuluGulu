...
import { fetchLentilleContext, friendlyError } from '~/utils/luogu-api'
...
<template>
  <div class="page-container" w-full h-full p="x-4 md:x-8 lg:x-16" pos="relative">
    ...
    >
      ...
      <p v-if="problemTitle" text="$bew-text-2">
        {{ pid }} {{ problemTitle }}
      </p>
    </div>
    ...
    <div
      ...
      p-8
      border="1 $bew-border-color"
      style="text-align: center; color: var(--bew-text-2); backdrop-filter: var(--bew-filter-glass-1)"
    >
    ...
    <Transition name="content-reveal">
      <div
        ...
        overflow="hidden"
      >
        <div v-for="(s, idx) in solutions" :key="s.lid" :style="{ '--row-index': idx }" border="b-1 $bew-border-color">
          <!-- Solution header row -->
          <div
          ...
          >
            <div flex="~ items-center gap-2" shrink-0>
              ...
              <span
                :style="{
                  color: s.author?.color ? `var(--bew-${s.author.color})` : 'var(--bew-text-1)',
                  fontWeight: 600,
            ...
            <div flex="~ items-center gap-3" shrink-0 style="font-size: 0.85em; color: var(--bew-text-3)">
              <span
                v-if="s.verified"
                text="xs"
                px-2
                py-0.5
                rounded-full
                style="background: var(--bew-success-color-20); color: var(--bew-success-color); font-weight: 600"
              >认证</span>
              <span flex="~ items-center gap-1"><span style="display: contents" v-html="renderIcon('mingcute:arrow-up-line', 14)" />{{
                s.upvote
              }}</span>
              ...
              <span
                style="display: contents; color: var(--bew-text-3)"
                v-html="renderIcon(expandedLid === s.lid ? 'mingcute:arrow-up-line' : 'mingcute:arrow-down-line', 16)"
              />
            </div>
          </div>
          ...
          <Transition name="page-fade">
            <div v-if="expandedLid === s.lid" p="x-6 y-4" bg="$bew-fill-1">
              <div
                ...
                v-html="parseMarkdownContent(s.content || '*(无内容)*')"
              />
            </div>
          </Transition>
        </div>
    ...
    <div v-if="!loading && solutions.length > 0 && solutions.length < solTotal" text="center" mt-4 mb-4>
      ...
      >
        加载更多 ({{ solutions.length }}/{{ solTotal }})
      </button>
    </div>

    <div
      v-if="!loading && !errorMsg && solutions.length === 0"
      bg="$bew-content"
      rounded="$bew-radius"
      p-8
      border="1 $bew-border-color"
