    ...
    <Transition name="content-reveal">
      ...
      >
        <div
          p="x-6 y-3"
          bg="$bew-fill-1"
          style="font-size: var(--bew-base-font-size); color: var(--bew-text-1); font-weight: 700"
        >
          已通过 {{ passed.length }} 题
        </div>
        <div
          v-for="(p, idx) in passed"
          :key="p.pid"
        ...
        >
          <span
            style="width: 80px; font-family: monospace; font-size: 0.85em; color: var(--bew-text-3); flex-shrink: 0"
          >{{ p.pid }}</span>
          ...
          <span
            ...
            py-0.5
            rounded-full
            :style="{ background: `${diffColor(p.difficulty || 0)}20`, color: diffColor(p.difficulty || 0) }"
          >{{ diffLabel(p.difficulty || 0) }}</span>
        </div>
      </div>
    </Transition>
    <div
      v-if="!loading && passed.length === 0 && submitted.length === 0"
      bg="$bew-content"
      rounded="$bew-radius"
      p-8
